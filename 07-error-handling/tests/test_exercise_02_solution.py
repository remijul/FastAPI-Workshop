"""
Tests pour la solution de l'exercice 2.
"""

import os
import sys
from fastapi.testclient import TestClient


def setup_module():
    """Nettoie la base avant les tests."""
    db_path = "databases/exercise_02.db"
    if os.path.exists(db_path):
        os.remove(db_path)
    
    # Forcer le rechargement
    modules_to_delete = [
        'exercises.exercise_02_solution.main',
        'exercises.exercise_02_solution.database',
        'exercises.exercise_02_solution.repositories',
        'exercises.exercise_02_solution.services',
        'exercises.exercise_02_solution.routes',
        'exercises.exercise_02_solution.models',
        'exercises.exercise_02_solution.exceptions'
    ]
    for module in modules_to_delete:
        if module in sys.modules:
            del sys.modules[module]


def test_create_room():
    from exercises.exercise_02_solution.main import app
    client = TestClient(app)
    
    room_data = {
        "name": "Chambre Deluxe",
        "capacity": 2,
        "price_per_night": 150.0
    }
    response = client.post("/rooms", json=room_data)
    assert response.status_code == 201
    data = response.json()
    assert data["name"] == "Chambre Deluxe"
    assert data["available"] is True


def test_get_room_not_found():
    from exercises.exercise_02_solution.main import app
    client = TestClient(app)
    
    response = client.get("/rooms/9999")
    assert response.status_code == 404
    data = response.json()
    assert data["error"] == "Room Not Found"


def test_create_reservation():
    from exercises.exercise_02_solution.main import app
    client = TestClient(app)
    
    room_data = {"name": "Chambre Standard", "capacity": 2, "price_per_night": 100.0}
    room_response = client.post("/rooms", json=room_data)
    room_id = room_response.json()["id"]
    
    reservation_data = {
        "room_id": room_id,
        "guest_name": "Alice Dupont",
        "nights": 3
    }
    response = client.post("/reservations", json=reservation_data)
    assert response.status_code == 201
    data = response.json()
    assert data["total_price"] == 300.0


def test_reservation_room_not_available():
    from exercises.exercise_02_solution.main import app
    client = TestClient(app)
    
    room_data = {"name": "Chambre Test", "capacity": 2, "price_per_night": 80.0}
    room_response = client.post("/rooms", json=room_data)
    room_id = room_response.json()["id"]
    
    # Première réservation
    reservation_data = {
        "room_id": room_id,
        "guest_name": "Charlie",
        "nights": 1
    }
    client.post("/reservations", json=reservation_data)
    
    # Deuxième réservation (devrait échouer)
    reservation_data2 = {
        "room_id": room_id,
        "guest_name": "Diana",
        "nights": 1
    }
    response = client.post("/reservations", json=reservation_data2)
    assert response.status_code == 400
    assert response.json()["error"] == "Room Not Available"